...
We have made all the log files of our evaluation available in this [link]([https://hkustconnect-my.sharepoint.com/:u:/g/personal/mamt_connect_ust_hk/EcaM0uwNAjRNtKaLtpFjnfYBiUym7EUaL_0aC20mj9dcyA?e=OSgDQT](https://hkustconnect-my.sharepoint.com/:u:/g/personal/mamt_connect_ust_hk/EcaM0uwNAjRNtKaLtpFjnfYB5xCPLHPizRNDQwf7NS8erw?e=6Ws0n8), which is around 280 GB after decompressing the zip file. Please note that the words `reach` and `event` in folder names refer to logs collected for RA and ASRA, respectively. You can download and unzip this folder and use `check.sh` to replicate the results we collected in the evaluation process. We also added `replication-data.xlsx,` which contains all the data about success ratio, overhead, reproduction time, and statistical test results (Fisher exact and Wilcoxon Mann-Whittney).
...
We stored all the collected data for all the RQs except RQ3 in an Excel file named `replication-data.xlsx.` Below, we explain the related sheets in this file for each RQ separately.
...
For more detailed information about the execution time of every single run of the total ten runs, we have used the four sheets named `Monkey (RA/ASRA/Mole),` `Ape (RA/ASRA/Mole),` `Stoat (RA/ASRA/Mole),` and `FastBot2 (RA/ASRA/Mole)` to store all the data of 
